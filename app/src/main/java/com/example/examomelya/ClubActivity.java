...
public class ClubActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ListView lv = findViewById(R.id.lv_club);
    ...
    void refresh_screen(){
        new GetClubRowsTasks(this).execute();
    }

    public void add_btn_clicked(View view) {
        String name = ((EditText) findViewById(R.id.name_club)).getText().toString();
        String address = ((EditText) findViewById(R.id.address)).getText().toString();
        String quantity_participants = ((EditText) findViewById(R.id.quantity_participants)).getText().toString();

        DatabaseConnector databaseConnector = new DatabaseConnector(this);

        if (rowId > 0) { // редагування
            databaseConnector.editClubRow(rowId, name, address, quantity_participants);
            rowId = 0;
        } else { // додавання
            databaseConnector.insertClubRow(name, address, quantity_participants);
        }
        refresh_screen();
    ...
    public boolean onContextItemSelected(@NonNull MenuItem item){
        ...
        switch (item.getItemId()) {
            case MENU_CONTEXT_DELETE_ID: { // видалення елемента з БД
                ...
                DatabaseConnector databaseConnector = new DatabaseConnector(this);
                databaseConnector.deleteClubRow(rid); //
            ...
            case MENU_CONTEXT_EDIT_ID: { // Редактирование компонента БД
                ...
                String quantityParticipants = str.split(", ")[3];

                // Отображение полей текста для редактирования
                ((EditText) findViewById(R.id.name_club)).setText(name);
                ((EditText) findViewById(R.id.address)).setText(address);
                ((EditText) findViewById(R.id.quantity_participants)).setText(quantityParticipants);
